...
4. **HTTP.** `POST /email/{id}/relay` and any `/v1/**/relay` path **segment** → 403 `receive_only`. A message or attachment id that merely contains the substring `relay` is not a relay route.
