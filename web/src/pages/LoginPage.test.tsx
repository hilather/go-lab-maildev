...
describe("LoginPage", () => {
  ...
  it("signs in with a token and never stores it", async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/v1/session") && method === "GET") {
        if (!signedIn) {
          return json(401, { status: 401, title: "unauthenticated", detail: "authentication required", code: "unauthenticated", type: "urn:labmail:error:unauthenticated" });
        }
        return json(200, { id: "admin", role: "administrator", scopes: ["mail.read"], csrf: "csrf-1", expiresAt: "2099-01-01T00:00:00Z" });
      }
  ...
  it("can switch to username and password", async () => {
    const user = userEvent.setup();
    vi.stubGlobal("fetch", vi.fn(async () => json(401, { status: 401, title: "unauthenticated", detail: "x", code: "unauthenticated", type: "urn:labmail:error:unauthenticated" })));
    renderApp(<LoginPage />, { route: "/login" });
    await user.click(screen.getByLabelText(/Username and password/i));
