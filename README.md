...
Status: **foundation + fail-closed YAML + plain SMTP sink + bounded inbox + native `/v1` REST**. The `labmail` binary implements `version`, `help`, `validate`, `canonicalize`, `healthcheck`, and `serve` (SMTP plus management HTTP). There is **no MCP, auth, UI, maildev `/email` compat, or container image** yet.
...
| Management / UI / REST | 1080 | inspect captured mail (native `/v1` now; `/email` compat and `/mcp` later) |
...
`serve` binds SMTP from the compiled YAML (override with `--smtp-listen`) and native `/v1` management HTTP from `spec.listeners.management.address` (override with `--management-listen ADDR|off`). Accepted messages are parsed and stored in a bounded memory inbox (ULID ids, stacked caps, Wipe on shutdown). Probe readiness with `labmail healthcheck --url=http://127.0.0.1:1080/v1/health/ready`.
