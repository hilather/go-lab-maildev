...
STA-001 implements `internal/app.Service` (HTTP-less) plus `internal/snapshot` and `internal/audit`. API-001 implements `internal/capabilities` and `internal/control/rest`. COMPAT-001 mounts maildev `/email`, `/healthz`, and `/config` on the same management listener (`POST /email/:id/relay` is 403). SEC-001 implements `internal/auth` (lab static bearer, Basic→same principal, CSRF session). UI-001 embeds the React inbox (`internal/web` `go:embed` of `web/dist`; `make web-build`). `labmail serve` boots `app.Service`, binds SMTP against the live snapshot (MAIL/RCPT/DATA re-read it), binds native `/v1` plus the SPA at `/`, MCP, and compat when `compatEnabled` is true, and wipes the inbox on reset/shutdown. STORE-001 added `store.Memory`. SMTP-001a added the in-tree SMTP listener. CFG-001 implements `version`, `help`, `validate`, and `canonicalize`. FND-001 shipped `version` and `help` only.
