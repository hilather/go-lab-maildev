...
10. **HTML sanitizer** will not match DOMPurify byte-for-byte; golden tests and comparison-lab HTML asserts define the policy.
11. **Legacy MCP clients** need `allowLegacyClients: true` for MCPJungle until the gateway speaks 2026-07-28.
12. **No OAuth PRM** on `/mcp` (static bearer / basic), same class as TacLab ADR 0010.
13. **Comparison lab HTML** compares user-visible text, not pretty-printed markup.
