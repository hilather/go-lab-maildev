...
func (s *Server) dispatch(w http.ResponseWriter, r *http.Request, instance string, actor app.Actor, rt compiledRoute, params map[string]string) {
	...
	case capabilities.MessagesGet:
		s.handleGetMessage(w, r, instance, ctx, actor, params["id"])
	case capabilities.MessagesRaw:
		s.handleRaw(w, r, instance, ctx, actor, params["id"])
	case capabilities.MessagesHTML:
		s.handleHTML(w, r, instance, ctx, actor, params["id"])
	case capabilities.MessagesPreview:
		s.handlePreview(w, r, instance, actor, params["id"])
	case capabilities.MessagesDelete:
		s.handleDeleteMessage(w, r, instance, ctx, actor, params["id"])
	case capabilities.MessagesClear:
	...
	case capabilities.MessagesExtract:
		s.handleExtract(w, r, instance, ctx, actor, params["id"])
	case capabilities.AttachmentsGet:
		s.handleAttachment(w, r, instance, ctx, actor, params["id"], params["attId"])
