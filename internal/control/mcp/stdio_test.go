...
func TestStdioDevAdapterSameRegistry(t *testing.T) {
	...
	if !seen["mail_version_get"] || !seen["mail_change_apply"] {
		t.Fatalf("stdio missing core tools: %v", seen)
	}
	res := callTool(t, cs, "mail_version_get", map[string]any{})
	if res.IsError {
...
func TestStdioListenURIOnlyNotify(t *testing.T) {
	...
	if err := cs.Subscribe(ctx, &sdk.SubscribeParams{URI: resourceMessages}); err != nil {
		t.Fatalf("stdio subscribe: %v", err)
	}
	// Wait for the listen stream to register before the first insert.
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		insertMail(t, svc, "stdio-listen", "body")
		select {
		case uri := <-got:
