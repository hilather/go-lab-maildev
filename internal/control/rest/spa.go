...
// import internal/web; cmd/labmail wires Config.UI.
...
// reservedManagementPath keeps /v1, MCP, and compat off the SPA fallback so a
...
func reservedManagementPath(p string) bool {
	...
	case p == "/mcp" || strings.HasPrefix(p, "/mcp/"):
		return true
	case p == "/email" || strings.HasPrefix(p, "/email/"):
		return true
