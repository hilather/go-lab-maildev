...
// HasScope reports whether p grants want. mail.admin satisfies every scope.
func (p Principal) HasScope(want string) bool {
	return HasScope(p.Scopes, want)
}

// HasScope reports whether scopes grant want. mail.admin satisfies every scope.
func HasScope(scopes []string, want string) bool {
	if want == "" {
		return true
	}
	for _, s := range scopes {
		if s == model.ScopeMailAdmin || s == want {
			return true
		}
	}
	return false
