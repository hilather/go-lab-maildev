...
LabMail is a **parity rewrite of the sink/inspection surface**, not a Node runtime clone and not a relay.
...
| Outbound | forbidden | `internal/maildev` rejects `outgoing-*` and `auto-relay*` before the container starts |
...
| POST | `/email/:id/relay/:relayTo?` | Outbound; **lab-forbidden** |
...
**Missing vs REST (parity gap we must close):** delete all, bulk delete, mark-all-read, HTML, source, download `.eml`, config, reload, health. Relay is also missing from MCP (good for us).
...
| Relay | Optional outgoing SMTP client — **out of scope** |
...
The 3.0 UI is a two-pane inbox: list + HTML/text/headers/source, search, keyboard shortcuts, command palette, dark mode, viewport preview, attachments, favicon badge. It already speaks REST. It must **not** keep Socket.IO or Relay.
...
- REST inspection: list/filter/skip, get (marks read), delete one/all, bulk delete, mark-all-read, html, source, download, attachment, config, healthz.
...
- Outbound relay and auto-relay (REST, MCP, UI, CLI, YAML).
- Node programmatic API and Express middleware embedding.
- AngularJS 2.x UI.
- Socket.IO (protocol and Node adapter).
