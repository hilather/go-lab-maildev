...
func TestCatalogLabelPolicy(t *testing.T) {
	...
	for _, m := range Metrics() {
		...
		seen[m.Name] = true
		if !strings.HasPrefix(m.Name, "labmail_") {
			t.Fatalf("metric %s missing labmail_ prefix", m.Name)
		}
	...
	required := []string{
		MetricSMTPSessionsTotal, MetricSMTPMessagesTotal, MetricSMTPSessionDuration,
		MetricSMTPSessionsActive, MetricStoreMessages, MetricStoreBytes,
		MetricStoreEvictions, MetricStoreWaiters, MetricHTTPRequestsTotal,
		MetricHTTPRequestDuration, MetricMCPCallsTotal, MetricAuthFailuresTotal,
		MetricAuditEventsTotal,
	}
...
func TestEventsHaveStableFields(t *testing.T) {
	...
	for _, e := range Events() {
		...
		if !strings.Contains(joined, "event") || !strings.Contains(joined, "request_id") ||
			!strings.Contains(joined, "smtp_code") || !strings.Contains(joined, "store_generation") {
...
func TestForbiddenAndAllowed(t *testing.T) {
	for _, k := range []string{"subject", "SUBJECT", "client_ip", "actor_id", "from", "to", "password", "authorization"} {
		if !ForbiddenLabel(k) {
			t.Fatalf("%s should be forbidden", k)
		}
	}
	for _, k := range []string{"result", "capability", "code_class", "tool", "reason", "event"} {
		if !AllowedLabel(k) {
